...
public extension Collection {
    ...
    func wb_forEachInParallel(_ each: (Self.Element) -> Void) {
    ...
    ///
    /// - Parameter size: grouped self.
    func wb_group(by size: Int) -> [[Element]]? {
...
public extension Collection where Index == Int {
    ...
    func wb_indices(where condition: (Element) throws -> Bool) rethrows -> [Index]? {
        var indicies: [Index] = []
        for (index, value) in lazy.enumerated() where try condition(value) {
            indicies.append(index)
        }
        return indicies.isEmpty ? indicies : nil
    }
    ...
    func wb_forEach(slice: Int, body: ([Element]) throws -> Void) rethrows {
